...
use crate::walk::{classify, readdir, Action, Context, Delegate, Error, Options, Outcome};
...
pub fn walk(
    ...
    let (status, file_type) = classify::root(
        worktree_root,
        &mut buf,
    ...
    if !can_recurse(buf.as_bstr(), status, file_type, delegate) {
        if buf.is_empty() && !matches!(file_type, Some(entry::Kind::Directory { .. })) {
    ...
    let _ = readdir::recursive(
        ...
        &mut buf,
        status,
        file_type.expect("exact type known by now"),
        &mut ctx,
        options,
        delegate,
        &mut out,
...
pub(super) fn emit_entry(
    ...
    if (!emit_empty_directories && kind == entry::Kind::EmptyDirectory
        || !emit_tracked && status == entry::Status::Tracked)
        || emit_ignored.is_none() && matches!(status, entry::Status::Ignored(_))
        || !emit_pruned && status.is_pruned()
    {
        return Action::Continue;
    }
